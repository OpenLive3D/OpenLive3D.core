...
function setCameraCallBack() {
  ...
  dbg.width = Math.floor(instantFrame.videoWidth * getCMV("CANVAS_RATIO"));
  dbg.height = Math.floor(instantFrame.videoHeight * getCMV("CANVAS_RATIO"));
        ...
        if (effectitem["parameters"]) {
          Object.keys(effectitem["parameters"]).forEach(function (parameter) {
            ...
            if (parameter == "color") {
              ...
              parColor.setAttribute(
                "value",
                effectitem["parameters"][parameter]
              );
              ...
              parrange.onchange = function () {
                let newvalue = Math.floor(
                  (parrange.value / 1000) * setrange + parArr[1]
                );
              ...
              parval.onchange = function () {
                ...
                let newvalue = Math.floor(
                  ((parval.value - parArr[1]) * 1000) / setrange
                );
    ...
    for (let i = 0; i < confmodifier.length; i++) {
      ...
      if (getBinaryCM().includes(configitem["key"])) {
        ...
        item.onclick = function myFunction() {
          ...
          if (configitem["key"] === "ENABLE_RIMLIGHT") {
            setRimlightEnabled(item.checked);
          }
        };
        ...
        let setvalue =
          ((getCMV(configitem["key"]) - configitem["range"][0]) * 1000) /
          setrange;
        item.setAttribute("value", setvalue);
        item.onchange = function () {
          let newvalue =
            (item.value / 1000) * setrange + configitem["range"][0];
        ...
        itemval.onchange = function () {
          ...
          let newvalue =
            ((itemval.value - configitem["range"][0]) * 1000) / setrange;
...
function createMoodLayout() {
  ...
  for (let i = 0; i < availableTrackingMode.length; i++) {
    ...
    handobj.onclick = function () {
      if (
        getCMV("IN_TRACKING_MODE_SELECT") ||
        ...
        if (keys[ikey]) {
          Object.keys(keys[ikey]).forEach(function (key) {
            ...
            jsonItem.innerHTML =
              getL(key) +
              ": " +
              Math.floor(keys[ikey][key] * 1000) / 1000 +
              "<br/>";
...
function raiseAlert(vistate, mlstate) {
  if (vistate > 0 || mlstate > 0) {
    ...
    if (vistate == 3) {
      alerttext.innerHTML = getL(
        "ALERT: Full Screen / Wrong Tab<br/>Browser will stop rendering when other program enters full screen!"
      );
    } else if (mlstate == 3) {
      alerttext.innerHTML = getL(
        "ALERT: Error<br/>ML loop stop running, might need to restart to validate."
      );
    } else if (mlstate == 2 || vistate == 2) {
      alerttext.innerHTML = getL(
        "ALERT: Hardware Acceleration<br/>ML loop is running extremely slow, check if hardware acceleration is opened."
      );
    } else if (mlstate == 1 && getCMV("HAND_TRACKING")) {
      alerttext.innerHTML = getL(
        "ALERT: Ultra Fast<br/>ML loop is running slowly, improve performance by using FACE-ONLY mode."
      );
    } else if (vistate == 1) {
      alerttext.innerHTML = getL(
        "ALERT: Slow<br/>Feel free to contact developer for more information."
      );
    }
  }
}
...
function drawSafari() {
  ...
  tmp2.innerHTML = getL(
    "Safari has no stable support for image processing in web-worker today (2023-10-13)"
  );
...
function isVisible(target) {
  ...
  return (
    obj.className.indexOf("w3-hide") == -1 && sidebar.style.display != "none"
  );
}
