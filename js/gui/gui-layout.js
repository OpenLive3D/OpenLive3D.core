...
function setupPostProcessing(useFXAA, useMSAA, useFSR) {
    ...
    if (fxaaRenderTarget) {
        fxaaRenderTarget.dispose();
        fxaaRenderTarget = null;
    }
    if (easuRenderTarget) {
        easuRenderTarget.dispose();
        easuRenderTarget = null;
    }
    if (ppQuad) {
    ...
    if (easuQuad) {
        easuQuad.geometry.dispose();
        easuQuad = null;
    }
    if (rcasQuad) {
        rcasQuad.geometry.dispose();
        rcasQuad = null;
    }
    ppScene = null;
    ppCamera = null;
    ...
    if (useFSR && typeof setupFSR === 'function') {
        ...
        if (useFXAA) {
            ...
            uniforms.convertToSRGB = {
                value: true
            }; // Final pass, output sRGB
        }
